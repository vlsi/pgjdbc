...
* fix: a connection that sets `assumeMinServerVersion` from 9.0 to 11.x reaches a session that permits only specific queries again, such as one opened with `options=-c gp_retrieve_conn=true`. Since 42.7.5 the driver delivered `extra_float_digits` only through a post-authentication `SET extra_float_digits = 3`, which such a session rejects, so the connection failed during initialization. The driver now sends the value in the startup packet, as 42.7.3 did, and it costs no extra round trip. On a 12+ server reached with `assumeMinServerVersion` from 9.0 to 11.x, `extra_float_digits` now reads 3 rather than the server default of 1; float output is unchanged, because every value above 0 selects the same shortest form [Discussion #4306](https://github.com/pgjdbc/pgjdbc/discussions/4306) [PR #4308](https://github.com/pgjdbc/pgjdbc/pull/4308)
