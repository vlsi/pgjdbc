...
### Fixed
* fix: `VisibleBufferedInputStream.available()` now counts what the socket is holding on top of what is buffered, instead of reporting the buffered bytes alone whenever there were any. A caller sizing its next read from that number no longer needs an extra round trip per buffer [PR #4376](https://github.com/pgjdbc/pgjdbc/pull/4376)
