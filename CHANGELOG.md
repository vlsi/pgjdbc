...
### Fixed
* fix: batching multi-statement SQL works again as it did in 42.7.12. 42.7.13 broke it twice over: `PreparedStatement.addBatch()` threw `ClassCastException` from `CompositeQuery` to `SimpleQuery` before sending anything, and `Statement.addBatch(String)` began refusing such SQL outright. Both stopped batches that had been running, because the outcome depends on the statements: an entry whose statements all report an update count overruns its single update-count slot with `Too many update results were returned`, as it always did, while an entry holding a `RETURNING` sub-statement executes, since that sub-statement returns rows instead of a row count. Note that the overrun is noticed only on the second `CommandComplete`, so the entry's statements have already run when it is reported as `EXECUTE_FAILED` — a 42.7.13 caller trades that surprise for a refusal, and gets it back here. Full support for multi-statement entries is a separate change. This reverts the refusal added by [PR #4014](https://github.com/pgjdbc/pgjdbc/pull/4014) [Issue #4349](https://github.com/pgjdbc/pgjdbc/issues/4349) [PR #4350](https://github.com/pgjdbc/pgjdbc/pull/4350)
* fix: a batch that forces an intermediate Sync to avoid a receive-buffer deadlock now records adaptive fetch for the results it reads there, so the first cursor of such a batch no longer falls back to the default fetch size [Issue #4349](https://github.com/pgjdbc/pgjdbc/issues/4349) [PR #4350](https://github.com/pgjdbc/pgjdbc/pull/4350)
