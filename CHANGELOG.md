...
### Added
* feat: a batch entry may now hold several SQL statements. JDBC gives each `addBatch()` call one update count, so the statements of an entry are collapsed onto it: the entry reports `0` when every statement of it changed nothing, and `Statement.SUCCESS_NO_INFO` when any of them changed rows, since no single figure describes them all. A failure part-way through an entry fails the entry as a whole. `Statement.addBatch(String)` needs `preferQueryMode=extended` (the default) for this, because below it the entry travels as one `Query` message and the server, not the driver, decides how many statements it holds [Issue #4349](https://github.com/pgjdbc/pgjdbc/issues/4349)
