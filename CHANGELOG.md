...
### Added
* feat: add the `pgjdbc.protocolHardeningMode` system property (`fail` | `disable`, default `fail`) as an escape hatch for the message-length ceilings the driver applies where the protocol fixes none. Raising the individual connection property is the better answer, since it keeps the remaining ceilings in force [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015) [PR #4016](https://github.com/pgjdbc/pgjdbc/pull/4016)
* feat: add the `maxServerTextMessageSize` connection property, the ceiling the driver applies to `ErrorResponse`, `NoticeResponse`, `CommandComplete`, `ParameterStatus` and `NotificationResponse`, where the protocol fixes none. It is unset by default, and the built-in 64 MB ceiling clears any workload that already works. `RowDescription`, `NegotiateProtocolVersion` and the pre-authentication token messages are bounded by fixed ceilings instead, since none of them varies with the workload [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015) [PR #4016](https://github.com/pgjdbc/pgjdbc/pull/4016)
* feat: add the `maxCopyDataSize` connection property, the ceiling the driver applies to a single `CopyData` message. It bounds logical and physical replication too, since both arrive in that unit. It is unset by default, and the built-in 64 MB ceiling clears any workload that already works [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015) [PR #4016](https://github.com/pgjdbc/pgjdbc/pull/4016)

### Fixed
* fix: each GSS token exchange is now bounded to 64 round-trips, and each token to the length libpq and the backend accept. A zero-length token is a legal continuation, so a server that answered every token with another one kept the client looping before any credentials had been exchanged. The encryption handshake and the authentication handshake are separate loops, and neither was bounded; an encrypted packet that declared a length over 16 kB used to reallocate the receive buffer to whatever the wire asked for [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015) [PR #4016](https://github.com/pgjdbc/pgjdbc/pull/4016)
* fix: the v3 protocol reader no longer trusts wire-provided lengths. A desynced or hostile backend could previously drive a ~1.7 GB allocation followed by an indefinite socket read, leaving the connection stuck and un-poolable. Every length-prefixed message is now bounded, message envelopes are tracked and verified, C-string scans cannot run unbounded, and a violation marks the connection broken so a pool discards it instead of handing it to the next caller. A length that is impossible on a conforming backend always closes the connection. The ceilings pgjdbc chose itself are set generously enough to clear any workload that already works, and each is a connection property that can be raised [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015) [PR #4016](https://github.com/pgjdbc/pgjdbc/pull/4016)
* fix: a result set no longer desyncs the connection when a row exceeds `maxResultBuffer`. The limit was checked after the row's length and field count had been read, so the remaining body stayed on the wire; the driver reported the error, carried on, and read the next message header out of the middle of the abandoned row. The check now runs before any of the body is read, the row is skipped, and the connection stays usable — the query still fails, but with `COMMUNICATION_ERROR` rather than a spurious I/O error [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015) [PR #4016](https://github.com/pgjdbc/pgjdbc/pull/4016)
* fix: a connection whose read was interrupted mid-message is no longer reused. A network failure partway through a backend message left the stream positioned inside that message, and the next query read its first message header out of the abandoned body. The driver now verifies that it sits on a message boundary before reading each message type, and marks the connection broken when it does not, so `isValid()` reports the failure instead of a later query returning nonsense [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015) [PR #4016](https://github.com/pgjdbc/pgjdbc/pull/4016)
