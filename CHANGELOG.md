...
* feat: add the `maxServerTextMessageSize` connection property, the ceiling the driver applies to `ErrorResponse`, `NoticeResponse`, `CommandComplete`, `ParameterStatus` and `NotificationResponse`, where the protocol fixes none. It is unset by default, and the built-in 64 MB ceiling clears any workload that already works. `RowDescription` and the pre-authentication token messages are bounded by fixed ceilings instead, since neither varies with the workload [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015) [PR #4016](https://github.com/pgjdbc/pgjdbc/pull/4016)
