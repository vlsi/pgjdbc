...
### Fixed
* fix: discarding bytes no longer fails with a `SocketTimeoutException` on a connection whose `SO_TIMEOUT` the driver set for its own purposes. A replication stream is one such connection, where the socket timeout comes from the status interval, 10 seconds unless the caller passes another value to `withStatusInterval`. `VisibleBufferedInputStream.skip` handed the count to the wrapped stream's `skip` and let that exception through, while a read on the same connection waits out the timeout unless the caller requested one through `setNetworkTimeout`. Discarding now goes through the driver's own buffer, so it waits out the timeout too and no longer depends on how a stream supplied through the `socketFactory` property implements `skip` [PR #4370](https://github.com/pgjdbc/pgjdbc/pull/4370)
* fix: `PGStream.skip` no longer loops forever when the connection drops while the driver is discarding bytes, and throws `EOFException` instead. A zero from `InputStream.skip` does not mean end of stream. Before this release the zero could come from a stream supplied through the `socketFactory` property with data still coming. The driver therefore reads a byte and treats only `-1` as the end [PR #4358](https://github.com/pgjdbc/pgjdbc/pull/4358)
