...
### Fixed
* fix: `Parser.parseSingleQuotes` recognises an `E''` literal that starts at the very beginning of the text it is given. The guard that checks the `E` is a token of its own, rather than the tail of a longer identifier, required a character before it, so the same literal was read one way at the start of the input and another way anywhere else [PR #4396](https://github.com/pgjdbc/pgjdbc/pull/4396)
