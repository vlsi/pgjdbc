...
### Fixed
* fix: `VisibleBufferedInputStream.skip` now skips nothing and returns `0` for a count of zero or less, as `InputStream.skip` requires. A negative count used to move the read position below the start of the buffer, after which `available()` over-reported and the next read threw `ArrayIndexOutOfBoundsException` instead of an `IOException` the driver can report [PR #4359](https://github.com/pgjdbc/pgjdbc/pull/4359)
