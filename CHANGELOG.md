...
### Changed
* fix: backend message lengths are checked before anything is sized from them. Each length is checked against the message layout and against a limit for its type. The lengths inside `DataRow` and `FunctionCallResponse` must fit the message that carries them, and the `NegotiateProtocolVersion` option count is bounded. A hostile or out of sync backend could previously force an allocation of up to 2 GiB from a five-byte header [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015)
* fix: the read buffer is limited to 32 MiB and grows once per request rather than once per socket read. It previously doubled on every read while a large request was outstanding. A peer that declared a large length and sent the body slowly could keep it doubling, until the doubling from 1 GiB overflowed to a negative size [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015)
* fix: a zero-length `CopyData` body is accepted, as it is by libpq. An assertion previously rejected it, so it failed only on JVMs started with `-ea` [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015)
* fix: GSS packet lengths are bounded. The driver previously sized its buffer from a length that arrives before authentication. The encrypted transport is limited to a 16380-byte payload and the handshake to a 65532-byte token, the limits libpq enforces from `PQ_GSS_MAX_PACKET_SIZE` and `PQ_GSS_AUTH_BUFFER_SIZE` [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015)
* fix: a connection is no longer reusable after the driver refuses a backend length or count. The refusal marks the stream broken, so `Connection.isClosed()` reports it closed and a pool that tests on borrow discards it. The socket is dropped with `SO_LINGER 0` [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015)
* fix: a protocol violation before authentication is reported as `PSQLException` with SQLState `08P01` rather than as a plain I/O error, and is no longer retried under `sslMode=allow` [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015)
...
* fix: before reading the next message type, the driver checks that the previous backend message ended exactly where its declared length put it. It refuses a message whose body was read short of that end or past it, instead of misreading the messages that follow [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015)
* fix: the authentication loop, the GSS encryption handshake, and the GSS authentication handshake each stop after 64 round trips. None of the three was bounded before: a zero-length GSS token is a valid continuation, and a server that repeated its password request was sent a password every time [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015)
* fix: an `ErrorResponse` read before authentication is limited to 30000 bytes, the limit libpq applies to messages outside `VALID_LONG_MESSAGE_TYPE`, rather than the 32 MiB an authenticated connection buffers. A C string is scanned only as far as the end of the message that carries it; the scan previously ran on until the read buffer reached its maximum. Both were allocations an unauthenticated peer could cause [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015)
* fix: an `ErrorResponse` or `NoticeResponse` longer than 32 MiB is truncated to that size rather than refused. The body past the limit is drained and the shortened message is delivered, so the connection stays usable. The fields before the truncation point are kept; later ones, such as the query text, are lost [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015)
* fix: the read buffer returns to its initial 8192 bytes once the message that grew it is drained, so a connection no longer retains 32 MiB after one large error [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015)
* fix: exceeding `maxResultBuffer` closes the connection. The row that takes the count past the limit is never read, so the stream is left off a message boundary. The connection previously stayed open on a stream the driver could no longer read. The error still carries SQLState `08S01`, and no second exception is chained to it [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015)
