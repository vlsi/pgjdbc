...
### Added
* feat: add the `pgjdbc.protocolHardeningMode` system property (`fail` | `disable`, default `fail`) as an escape hatch for some of the message-length limits the driver applies. `disable` lifts the `maxServerTextMessageSize` limit after authentication, the built-in `CopyData` limit while `maxCopyDataSize` is unset, and the limit on one string inside a message. It does not lift the `RowDescription` limit, a `maxCopyDataSize` you set, or any message-length limit applied before authentication. Prefer raising the connection property of the one limit that fails, because that keeps the other limits in force [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015) [PR #4016](https://github.com/pgjdbc/pgjdbc/pull/4016)
* feat: add the `maxServerTextMessageSize` connection property, the largest `ErrorResponse`, `NoticeResponse`, `CommandComplete`, `ParameterStatus` or `NotificationResponse` the driver accepts, where the protocol fixes no maximum of its own. It is unset by default, which applies a built-in limit of 64 MB (64000000 bytes), large enough for any workload that works today. `RowDescription`, `NegotiateProtocolVersion` and the pre-authentication token messages have fixed limits instead, since none of them varies with the workload [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015) [PR #4016](https://github.com/pgjdbc/pgjdbc/pull/4016)
* feat: add the `maxCopyDataSize` connection property, the largest single `CopyData` message the driver accepts. It limits logical and physical replication too, since both arrive as CopyData messages. It is unset by default, which applies a built-in limit of 64 MB (64000000 bytes), large enough for any workload that works today [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015) [PR #4016](https://github.com/pgjdbc/pgjdbc/pull/4016)

### Fixed
* fix: the GSS encryption handshake and the GSS authentication handshake each stop after 64 round-trips, and each token is limited to the length libpq and the backend accept. The two handshakes are separate loops, and neither had a limit. A zero-length token is a valid continuation, so a server that sent another token after every token kept the client looping before any credentials had been exchanged. An encrypted packet that declares a length outside 1 to 16380 bytes no longer resizes the receive buffer to the declared length. The driver closes the connection and fails with `Protocol error. GSS encrypted packet has invalid length N (expected between 1 and 16380 bytes).` [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015) [PR #4016](https://github.com/pgjdbc/pgjdbc/pull/4016)
* fix: the v3 protocol reader no longer trusts wire-provided lengths. A backend that was out of sync or hostile could previously drive an allocation of about 1.7 GB followed by an indefinite socket read, leaving the connection stuck and unfit to return to a pool. Every length-prefixed message now has a limit, each declared message length is checked against the bytes its reader consumed, a C-string scan stops at a limit, and a violation marks the connection broken so a pool discards it instead of handing it to the next caller. A length that is impossible on a conforming backend always closes the connection. The limits pgjdbc chose itself are large enough for any workload that works today, and the ones that vary with the workload are connection properties that can be raised [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015) [PR #4016](https://github.com/pgjdbc/pgjdbc/pull/4016)
* fix: `maxResultBuffer` rejects `0` and negative values when the connection opens, with an error that begins `The maxResultBuffer connection property must be a positive size, but its value is 0.` and SQLState `22023`. Previously `-1` was accepted as no limit, and `0` or any other negative value was accepted as a limit that every row with data exceeded, so each such row failed with an error that begins `Result set exceeded maxResultBuffer limit.` A connection string with `maxResultBuffer=-1` that used to connect without a limit now fails; leave the property unset instead. The new `maxCopyDataSize` and `maxServerTextMessageSize` properties share the same syntax and reject the same values [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015) [PR #4016](https://github.com/pgjdbc/pgjdbc/pull/4016)
* fix: `maxResultBuffer` rejects a value that is not a size when the connection opens, with `The maxResultBuffer connection property has the value V, which is not a valid size. Give a byte count such as 150M or a share of the heap with the p suffix, such as 10p.` and SQLState `22023`. Previously `10%` was accepted as no limit. A value whose number did not parse, such as `abcM`, failed with `Something unusual has occurred to cause the driver to fail. Please report this exception.`, SQLState `99999` and a `NumberFormatException` as the cause. A value ending in a character that is neither a digit nor a size suffix, such as `10x`, failed with `Received MaxResultBuffer parameter cant be parsed. Value received to parse: {0}` and SQLState `42601`. A connection string with a value such as `10%` that used to connect without a limit now fails, and code that matched the old messages or SQLStates must be updated. `maxCopyDataSize` and `maxServerTextMessageSize` reject the same values [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015) [PR #4016](https://github.com/pgjdbc/pgjdbc/pull/4016)
* fix: `maxResultBuffer` no longer counts the rows of a statement that failed on the server against the next statement. When a statement failed after some of its rows had arrived, such as a query that divides by zero in a later row, the running count was not reset, so the next statement on the connection could fail with `Result set exceeded maxResultBuffer limit` although its own rows fit within the limit. The count now restarts on every `ReadyForQuery`, so it covers one round trip to the server: usually a whole `execute` call, or one batch of the fetch size when the result is read through a cursor. The driver can split a batch, or a call that runs several statements, into several round trips. The defect dates from 42.2.10, which added the property [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015) [PR #4016](https://github.com/pgjdbc/pgjdbc/pull/4016)
* fix: a result set over `maxResultBuffer` no longer leaves the connection out of sync or closed. The limit was checked after the row's length and field count had been read, so the rest of the row stayed unread in the stream. The driver reported the error, continued, and read the next message header from the middle of the abandoned row. The check now runs before any of the row body is read, both for a row that is over the limit by itself and for a row that would take the rows already buffered past it. The row is skipped, and the connection stays usable. The query still fails, with `Result set exceeded maxResultBuffer limit.` and SQLState `08S01` rather than a spurious I/O error or a closed connection. Only the first such row raises an error, and later rows that can be skipped are skipped without an error until the next `ReadyForQuery`, so a result read through a cursor can report once per batch. A row whose unread body is over 64 MB (64000000 bytes) is too large to skip, so the driver closes the connection instead [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015) [PR #4016](https://github.com/pgjdbc/pgjdbc/pull/4016)
* fix: a connection whose read was interrupted mid-message is no longer reused. A network failure partway through a backend message left the stream positioned inside that message, and the next query read its first message header out of the abandoned body. The driver now verifies that the stream sits on a message boundary before it reads each message type, and marks the connection broken when the stream does not, with `Protocol error. Reading the <message type> message stopped with N bytes of its body unread, so the connection is no longer positioned on a message boundary.`, so `isValid()` reports the failure instead of a later query reading data from the abandoned message [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015) [PR #4016](https://github.com/pgjdbc/pgjdbc/pull/4016)
