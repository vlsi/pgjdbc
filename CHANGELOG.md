...
* fix: `getFloat()` on a `float8` or `numeric` value outside the `float` range now throws with SQLState `22003` instead of silently saturating (an overflow returned `±Infinity`, and a nonzero value that underflows returned `0.0`). This matches PostgreSQL's `float8`-to-`float4` cast, and `getObject(Float.class)` behaves the same. `getDouble()` on a `numeric` value beyond the `double` range likewise throws instead of returning `±Infinity`. A genuine `NaN` or `Infinity` still reads back unchanged. Code that relied on the saturated value must adjust.
