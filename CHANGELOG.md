...
### Added
* feat: a batch entry may now hold several SQL statements (`INSERT ...; DELETE ...`). JDBC gives each `addBatch()` call one update count, so the statements of an entry are collapsed onto it. The entry reports `0` only when every statement of it reported a row count of its own and all of those counts were zero; otherwise it reports `Statement.SUCCESS_NO_INFO`, since no single figure describes the entry. `SUCCESS_NO_INFO` therefore also covers a statement whose outcome the driver never learned: `TRUNCATE`, `SET` and the DDL family report no row count at all, `CREATE TABLE AS` reports the rows it wrote under a `SELECT` tag, and a `RETURNING` statement's count does not reach a batch that did not ask for generated keys. A failure part-way through an entry fails the entry as a whole, as it already did for `reWriteBatchedInserts`. Generated keys are not collected for a multi-statement entry, which has always been true of multi-statement `PreparedStatement`s. `Statement.addBatch(String)` needs `preferQueryMode=extended` (the default) or `extendedCacheEverything`; under `simple` and `extendedForPrepared` a non-parameterized entry travels as one `Query` message and the server, not the driver, decides how many statements it holds, so it is refused there [Issue #4349](https://github.com/pgjdbc/pgjdbc/issues/4349)
* fix: a failing `CallableStatement` batch now names the entry that failed. `CallableBatchResultHandler` discarded the rows a call returns without recording that the entry had finished, so no entry ever advanced: every failure was reported as `Batch entry 0` and no progress could be secured mid-batch [Issue #4349](https://github.com/pgjdbc/pgjdbc/issues/4349)

### Fixed
* fix: batching multi-statement SQL works again as it did in 42.7.12. 42.7.13 broke it twice over: `PreparedStatement.addBatch()` threw `ClassCastException` from `CompositeQuery` to `SimpleQuery` before sending anything, and `Statement.addBatch(String)` began refusing such SQL outright. Both stopped batches that had been running, because the outcome depends on the statements: an entry whose statements all report an update count overruns its single update-count slot with `Too many update results were returned`, as it always did, while an entry holding a `RETURNING` sub-statement executes, since that sub-statement returns rows instead of a row count. Full support for multi-statement entries is a separate change [Issue #4349](https://github.com/pgjdbc/pgjdbc/issues/4349)
* fix: a batch that forces an intermediate Sync to avoid a receive-buffer deadlock now records adaptive fetch for the results it reads there, so the first cursor of such a batch no longer falls back to the default fetch size [Issue #4349](https://github.com/pgjdbc/pgjdbc/issues/4349)
* fix: batching a `PreparedStatement` whose SQL holds several statements (`INSERT ...; DELETE ...`) no longer throws `ClassCastException` from `CompositeQuery` to `SimpleQuery`. 42.7.13 threw it before sending anything, which stopped batches that had worked: an entry whose statements all report an update count still overruns its single slot with `Too many update results were returned`, as in 42.7.12, while an entry with a `RETURNING` sub-statement executes as it did before. Full support for multi-statement entries is a separate change [Issue #4349](https://github.com/pgjdbc/pgjdbc/issues/4349)
* fix: `PreparedStatement.addBatch()` on multi-statement SQL (`INSERT ...; DELETE ...`) no longer throws `ClassCastException` from `CompositeQuery` to `SimpleQuery` [Issue #4349](https://github.com/pgjdbc/pgjdbc/issues/4349)
