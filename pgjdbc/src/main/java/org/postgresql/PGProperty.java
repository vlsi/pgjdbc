...
public enum PGProperty {
   ...
   * NotificationResponse the driver accepts, in bytes. The protocol fixes no maximum for these
   * server-generated text messages, so the driver applies its own limit of 64 MB (64000000
   * bytes) by default. Raise it if the backend legitimately sends larger notices or error
   * details. The system property {@code -Dpgjdbc.protocolHardeningMode=disable} switches the
   * limit off, except on an ErrorResponse that arrives before authentication. Suffixes are
   * decimal, so {@code 64M} is 64000000 bytes.
   */
  MAX_SERVER_TEXT_MESSAGE_SIZE(
      "maxServerTextMessageSize",
      null,
      "Specifies the largest ErrorResponse, NoticeResponse, CommandComplete, ParameterStatus or NotificationResponse the driver accepts. When unset, the driver applies a built-in limit of 64 MB (64000000 bytes). After authentication, -Dpgjdbc.protocolHardeningMode=disable switches the limit off, whether or not this property is set. Can be specified as a size or a percent of heap memory."),
