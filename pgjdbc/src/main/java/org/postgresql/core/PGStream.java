...
public class PGStream implements Closeable, Flushable {
   ...
   * to bound it. The pre-authentication ErrorResponse is outside the rule:
   * {@code maxServerTextMessageSize} bounds it, far above a single C-string, and its body is
   * read as a block rather than scanned.
   */
   ...
   * {@link #readPreAuthMessageLength(String, int, int) readPreAuthMessageLength} (any overload),
   ...
   * {@link #readPreAuthMessageLength(String, int, int, String)}, which no mode switches off.
   *
   ...
   * from the protocol.</p>
   *
   * <p>{@link ProtocolHardeningMode#DISABLE} does not switch this limit off. The peer is not yet
   * authenticated, so a limit that is too small is raised for the connection that needs it
   * rather than switched off for the whole JVM. This overload is for a limit that nothing
   * raises, and its error message states that the limit cannot be relaxed. Where a connection
   * property raises the limit, use {@link #readPreAuthMessageLength(String, int, int, String)}.</p>
   *
   * @param messageName protocol message name used in the error message
   * @param minLength inclusive minimum valid value of the length field; must be at least 4
  ...
  /**
   * Same as {@link #readPreAuthMessageLength(String, int, int)}, for a limit the user can
  ...
  public int readPreAuthMessageLength(String messageName, int minLength, int limit,
    ...
    if (len > limit) {
      throw markBroken(new IOException(GT.tr(
          ...
          messageName, String.valueOf(len), String.valueOf(limit), propertyName)));
    }
    beginMessage(messageName, len);
    return len;
  }
