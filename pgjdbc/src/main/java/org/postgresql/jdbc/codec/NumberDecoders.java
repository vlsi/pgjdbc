...
final class NumberDecoders {
  ...
  /**
   * Refuses a numeric literal that carries a non-ASCII character, so that a fullwidth
   * {@code "１２３"} does not decode to 123 where {@code int4in} and {@code numeric_in} reject it.
   *
   * <p>Every grammar these codecs accept is ASCII throughout — digits, sign, decimal point, exponent
   * marker — so {@link NumberParser#requireAsciiLiteral}'s character range test covers it, and the JDK
   * parser still owns the grammar and the error message for everything else.</p>
   *
  ...
  static void requireAsciiLiteral(String text) {
    NumberParser.requireAsciiLiteral(text);
  }
