 ...
 * {@link BackpatchingByteArrayOutputStream}: the type OID, a back-patched length prefix, and the body. A field
...
public final class PgSQLOutputBinary extends PgSQLOutput {
   ...
   */
  private final BackpatchingByteArrayOutputStream sink;
   ...
   */
  public PgSQLOutputBinary(PgType type, PgCodecContext ctx, BackpatchingByteArrayOutputStream sink) throws SQLException {
    super(type, ctx);
  ...
  protected void writeFieldObject(Object value) throws SQLException, IOException {
    int slot = openField();
    CodecFormatSupport.writeBinary(sink, value, getCodec(), getCurrentType(), ctx);
    closeField(slot);
  }
}
